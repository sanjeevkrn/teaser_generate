...
class Batch(object):
    ...
    """
        ...
        self.stop_id = vocab.word2id(data.STOP_DECODING)  # id of the PAD token used to pad sequences
    ...
    def init_decoder_seq(self, example_list, hps):
        ...
        for ex in example_list:
            ex.pad_decoder_inp_targ(hps.max_dec_steps, self.stop_id)
...
class Batcher(object):
    def __init__(self, data_path, vocab, hps, single_pass=False, num_epoch=None):
      ...
      """
        ...
        self.input_gen = self.text_generator(
            data.example_generator(self._data_path, self._single_pass, num_epochs=self._num_epoch))
    ...
    def next_batch(self):
        ...
        inputs = []
        for _ in xrange(self._hps.batch_size):
            try:
                (article,
                 abstract) = self.input_gen.next()  # read the next example from file. article and abstract are both strings.
            except StopIteration:  # if there are no more examples:
                if self._single_pass and len(inputs) == 0:
        ...
        return Batch(inputs, self._hps, self._vocab)
